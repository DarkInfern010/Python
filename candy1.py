...
while 1:
    ...
    if(go):
        s.send((str('PRIVMSG Candy !ep1 \r\n').encode()))
        go=False
    #endif

    if (data.find(':Candy!Candy@root-me.org PRIVMSG DarkInfern010 :') != -1):
        numbers = data.split(':')[2]
        numbre1 = int(numbers.split('/')[0])
        numbre2 = int(numbers.split('/')[1])
        res = sqrt(numbre1) * numbre2
        envoie = str(res.__round__(2))
        s.send((str('PRIVMSG Candy !ep1 -rep '+envoie+'\r\n').encode()))
s.close()